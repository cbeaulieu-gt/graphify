...
class TestLoudChunkFailure:
    ...
    def _make_failing_extract(self, monkeypatch):
        """Patch _extract_with_adaptive_retry to always raise."""
        monkeypatch.setattr(
            llm,
            "_extract_with_adaptive_retry",
            lambda *a, **kw: (_ for _ in ()).throw(RuntimeError("charmap codec error")),
        )
    ...
    def test_no_false_alarm_when_all_chunks_succeed(self, monkeypatch, tmp_path, capsys):
        ...
        assert result.get("failed_chunks", 0) == 0
        captured = capsys.readouterr()
        # "failed" should NOT appear in a summary context (individual chunk
        # errors may appear; we check the final summary line specifically)
...
class TestSubstitutionValidation:
    ...
    def test_cp1252_would_fail_but_utf8_succeeds(self, tmp_path):
        ...
        try:
            ...
            # If it doesn't fail, the test content doesn't cover the issue —
            # fail loudly so the test author knows to update _UNICODE_CHARS.
            raise AssertionError(
                "Expected cp1252 encode to fail for chars → ✅ ≥, but it succeeded. "
                "Update _UNICODE_CHARS to include cp1252-incompatible characters."
            )
    ...
    def test_subprocess_encoding_kwarg_in_extract_files_direct(self, monkeypatch, tmp_path):
        ...
        _ENVELOPE_SIMPLE = {
            ...
            "result": json.dumps({
                "nodes": [{"id": "u_chunk", "label": "Unicode Chunk",
                           "file_type": "document", "source_file": "unicode_chunk.md"}],
                "edges": [], "hyperedges": [], "input_tokens": 1, "output_tokens": 1,
            }),
            "stop_reason": "end_turn",
            "usage": {"input_tokens": 1, "output_tokens": 1,
        ...
        # Confirm the unicode content was in the input (not truncated or replaced)
